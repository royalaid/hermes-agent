"""Source-level contract for the Windows desktop-update hand-off ack.

The locale-independent timestamp test that used to live here was purged
upstream (524c38a98a, low-value source-reading tests). The fork's ack test
stays: the torn-read race it guards is not reachable from a runtime test,
and the behavioral twin in tests/test_desktop_update_windows_marker_and_log.py
