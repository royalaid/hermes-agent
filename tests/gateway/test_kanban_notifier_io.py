...
def _count_sqlite_opens(monkeypatch):
    opens = []
    real_connect = kbc.sqlite3.connect

    def counted_connect(*args, **kwargs):
        opens.append((args, kwargs))
        return real_connect(*args, **kwargs)

    monkeypatch.setattr(kbc.sqlite3, "connect", counted_connect)
    return opens
...
def test_notifier_reopens_changed_wal_and_checkpointed_board_next_poll(
    ...
    def change_after_first_poll(poll_sleeps):
        if poll_sleeps != 1:
            return
        opens_after_initial_poll.append(len(opens))
    ...
    assert len(opens_after_initial_poll) == 1
    assert len(opens) > opens_after_initial_poll[0]
...
def test_notifier_invalidates_cache_for_subscription_remove_and_add(
    ...
    def change_subscription_after_poll(poll_sleeps):
        opens_by_poll.append(len(opens))
        if poll_sleeps == 1:
            assert kbn.remove_notify_sub(
                writer,
                task_id=first_task,
    ...
    assert len(opens_by_poll) == 3
    assert opens_by_poll[0] < opens_by_poll[1] < opens_by_poll[2]
...
def test_notifier_retries_after_board_query_failure(tmp_path, monkeypatch):
    """A swallowed board-open failure must not turn into a cached quiet poll."""
    db_path = tmp_path / "query-failure.db"
    monkeypatch.setenv("HERMES_KANBAN_DB", str(db_path))
    _seed_subscription(db_path, title="retry failed query")
    adapter = RecordingAdapter()
    runner = _make_runner(adapter)
    ...
    def fail_once(*args, **kwargs):
        ...
        attempts += 1
        if attempts == 1:
            raise OSError("simulated board-open failure")
        return real_connect(*args, **kwargs)

    monkeypatch.setattr(kbc, "connect", fail_once)
...
def test_notifier_does_not_cache_through_concurrent_writer(tmp_path, monkeypatch):
    ...
    runner = _make_runner(adapter)
    opens = _count_sqlite_opens(monkeypatch)
    real_list_notify_subs = kbn.list_notify_subs
    ...
    def list_with_concurrent_writer(conn, *args, **kwargs):
        ...
        thread.start()
        thread.join()
        opens_after_writer.append(len(opens))
        return rows

    monkeypatch.setattr(kbn, "list_notify_subs", list_with_concurrent_writer)
    poll_sleeps = _run_polls(monkeypatch, runner, 2)
    ...
    assert len(adapter.sent) == 1
    assert task_id in adapter.sent[0]["text"]
    assert len(opens_after_writer) == 1
    assert len(opens) > opens_after_writer[0]
...
def test_notifier_closes_poll_connections_on_shutdown(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(kbc.sqlite3, "connect", tracked_connect)
