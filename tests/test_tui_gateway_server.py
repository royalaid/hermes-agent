...
def test_session_create_cron_marker_is_live_only_and_scopes_agent_turn_context(monkeypatch):
    ...
    persisted = {}
    identity = {"user_id": "cron-browser-user", "provider": "test-provider"}
    ...
    try:
        ...
        marked_session["transport"] = types.SimpleNamespace(auth_identity=identity)
        ...
        try:
            ...
            assert get_session_env(
                "HERMES_BROWSER_CONTROL_PRINCIPAL"
            ) == _principal_digest(identity)
            assert (
