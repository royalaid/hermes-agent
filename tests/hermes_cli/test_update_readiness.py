...
@pytest.mark.parametrize(
    "outcome",
    ["success", "body-failure", "heartbeat-loss"],
)
@pytest.mark.windows_only
def test_windows_cmd_update_orders_all_transaction_cleanup(
    monkeypatch, outcome: str
) -> None:
    """Finalize the command result before mutation teardown in every outcome."""
    ...
    class FakeHeartbeat:
        lost = outcome == "heartbeat-loss"
        loss_reason = (
            "injected update quiesce lease loss"
            if outcome == "heartbeat-loss"
            else None
        )
    ...
    monkeypatch.setattr(
        ...
        lambda code, _detail: events.append(f"receipt-{code}"),
    )
    monkeypatch.setattr(
        cli_main,
    ...
    monkeypatch.setattr(
        cli_main,
        "_release_update_quiesce_lease",
        lambda root, value: events.append("lease-release")
        or (root == cli_main.PROJECT_ROOT and value is lease),
    )
    args = SimpleNamespace(
        preflight=False,
        drain=False,
    ...
    assert events == [
        ...
        "heartbeat-start",
        "body",
        "receipt-0" if outcome == "success" else "receipt-1",
        "heartbeat-stop",
        "job-disarm",
        "gateway-resume",
        "lease-release",
