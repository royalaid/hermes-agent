...
def test_self_test_has_a_log_arm() -> None:
    ...
    # The guard grows a term whenever a new self-test arm lands (the relaunch
    # environment arm did). Assert the invariant -- every self-test switch
    # relaxes the -InstallRoot requirement -- not one frozen spelling.
    guard = next(
        line
        for line in src.splitlines()
        if line.startswith("if (-not $SelfTestUi") and "$InstallRoot" in line
    )
    for switch in ("$SelfTestUi", "$SelfTestPipeDrain", "$SelfTestLog"):
        assert f"-not {switch} -and" in guard
    assert guard.rstrip().endswith("-and -not $InstallRoot) {")
