...
# ── Update-marker ownership ───────────────────────────────────────────────
# ``hermes_cli.update_lock`` reads the install-wide update marker through this
# module because the gate is the only stdlib-only place both the pre-import MCP
# bridge and the updater can share. The bridge lease above (``MARKER_NAME``) and
# the update marker below are DIFFERENT artifacts; keep their path helpers
# distinct. Wire format is ``<pid>\n<started_at>\n`` with an optional third
# ``handoff-bridge`` line, matched by apps/desktop/electron/update-marker.ts and
# scripts/desktop-update/windows.ps1.

UPDATE_MARKER_NAME = ".hermes-update-in-progress"
UPDATE_MARKER_MAX_AGE_SECONDS = 20 * 60


def update_marker_path(hermes_home: str | Path | None = None) -> Path:
    """Return the install-global update-marker path (see :func:`marker_path`)."""
    if hermes_home is None:
        root = get_default_hermes_root()
    else:
        root = Path(hermes_home).expanduser()
        if root.parent.name.casefold() == "profiles":
            root = root.parent.parent
    return root / UPDATE_MARKER_NAME
...
def live_update_marker_owner(
    ...
    *,
    ...
    path = Path(marker) if marker is not None else update_marker_path()
    try:
