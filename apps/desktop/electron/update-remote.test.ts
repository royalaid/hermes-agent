...
test('update checks force-refresh the exact origin tracking ref they read', () => {
  assert.deepEqual(updateBranchFetchArgs('fork-integration'), [
    'fetch',
    '--quiet',
    'origin',
    '+refs/heads/fork-integration:refs/remotes/origin/fork-integration'
  ])
})
