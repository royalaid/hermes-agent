...
test('resolveWindowsDevRelaunchAppPath preserves only a real dev app entry', () => {
  assert.equal(
    resolveWindowsDevRelaunchAppPath(true, ['electron.exe', 'C:\\Hermes\\apps\\desktop']),
    path.resolve('C:\\Hermes\\apps\\desktop')
  )
  assert.equal(resolveWindowsDevRelaunchAppPath(false, ['Hermes.exe', 'ignored']), undefined)
  assert.equal(resolveWindowsDevRelaunchAppPath(true, ['electron.exe', '--inspect']), undefined)
})
