...
function isShimLocked(shimPath) {
  return isFileLocked(shimPath)
}

/** Install resources that must be clear before updater mutation begins. */
...
async function releaseBackendLockForUpdate(updateRoot: string, tag = 'updates', deadline?: UpdateBlockerDeadline) {
  ...
  if (inventory.kind === 'blocked') {
    const pids = new Set([
      ...inventory.result.processes.map(holder => holder.pid),
      ...inventory.result.mcpBridges.map(holder => holder.pid),
      ...inventory.result.desktopPluginServices.map(holder => holder.pid)
    ])
...
//
// settleMs: updater uses a short tracked-backend settle (force-release handles
// foreign holders). Uninstall keeps the longer historical wait because it has
// no force-release fallback and proceeds even when unlocked is false.
async function releaseBackendLock(updateRoot, tag, options: { settleMs?: number } = {}) {
  if (!IS_WINDOWS) {
    return { unlocked: true }
  }
  ...
  const defaultSettleMs = tag === 'uninstall' ? 15_000 : 2_000
  const settleMs = Math.max(0, options.settleMs ?? defaultSettleMs)
  const deadlineMs = Date.now() + settleMs

  while (Date.now() < deadlineMs) {
    if (!isAnyInstallResourceLocked(updateRoot) && !isShimLocked(shim)) {
...
// `hermes update`). Instead we hand off and quit, so the orchestrator can run
...
async function applyUpdatesTransaction(
  ...
  try {
    ...
    if (launch.kind !== 'spawned') {
      throw new Error('Windows update transport changed after preflight')
    }
...
async function spawnPoolBackend(profile, entry, opts: { forceLocal?: boolean; poolKey?: string } = {}) {
  ...
  throwIfPoolBackendStartCancelled(backendPool, poolKey, entry)
  ...
  // silently for background profiles — so we only log while parked.
  {
    ...
    await waitForPoolBackendStartClearance(backendPool, poolKey, entry, updateGateDeps(), {
      onWaitTick: reason => {
        if (!poolAnnounced) {
          poolAnnounced = true
  ...
  child.once('error', error => {
    ...
    deletePoolBackendEntryIfCurrent(backendPool, poolKey, entry)
    rejectStart?.(error)
  })
...
// Abort a pooled backend's parked start work (update-gate clearance waits,
// retry sleeps) before the stopper runs. cancelPoolBackendStart() only fires
// while the entry is still the pool's current one for the key, and
// poolStopper.stop() evicts the entry synchronously — so the cancel has to
// happen first or a parked start would keep waiting on a backend nobody owns
// any more. A key with a stop already in flight has no entry left, and its
// first stop already cancelled it.
function cancelParkedPoolBackendStart(key) {
  const entry = backendPool.get(key)

  if (entry) {
    cancelPoolBackendStart(backendPool, key, entry)
  }
}

function stopPoolBackend(profile) {
  cancelParkedPoolBackendStart(profile)

  return poolStopper.stop(profile)
}

async function teardownPoolBackendAndWait(profile) {
  const keys = localProfilePoolKeys(profile)

  // Cancel every key first, then stop: a parked start must not observe a
  // sibling scope still pooled and keep the profile's HERMES_HOME busy.
  for (const key of keys) {
    cancelParkedPoolBackendStart(key)
  }

  await Promise.all(keys.map(key => poolStopper.stop(key)))
}
