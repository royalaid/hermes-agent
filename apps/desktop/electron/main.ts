...
import { isOfficialSshRemote, OFFICIAL_REPO_HTTPS_URL, updateBranchFetchArgs } from './update-remote'
import {
...
async function checkUpdates() {
  ...
  // update FETCH_HEAD without moving origin/<branch>, especially after a fork
  // force-push, which makes a stale checkout look current.
  const fetched = await runGit(updateBranchFetchArgs(branch), { cwd: updateRoot })
