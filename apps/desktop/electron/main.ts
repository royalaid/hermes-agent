...
import { OFFICIAL_REPO_HTTPS_URL, resolveUpdateRemote, updateBranchFetchArgs } from './update-remote'
import {
...
import { scanVenvBlockers, terminateMcpBridge } from './venv-blocker-scan'
...
async function checkUpdates(updateRoot = resolveUpdateRoot(), resolvedBranch = '') {
  ...
  // Refresh the exact remote-tracking ref read below. A source-only fetch may
  // update FETCH_HEAD without moving <remote>/<branch>, especially after a fork
  // force-push, which makes a stale checkout look current.
  const fetched = await runGit(updateBranchFetchArgs(remote.name, branch), { cwd: updateRoot })
...
async function applyUpdatesTransaction(opts = {}) {
  ...
  try {
    ...
    const preflight = await runWindowsHandoffPreflight(updateRoot, 'normal-update')
...
async function handOffWindowsBootstrapRecoveryTransaction(reason, updater) {
  ...
  try {
    ...
    // a crash and provoke a mid-recovery relaunch. The updater waits for this
    // exact Desktop PID and refuses mutation if it remains alive.
    isQuittingForHandoff = true
    setTimeout(() => {
      app.quit()
    }, UPDATE_HANDOFF_DWELL_MS)
