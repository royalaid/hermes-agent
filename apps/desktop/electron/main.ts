...
ipcMain.handle('hermes:api', async (_event, request) => {
  ...
  // A profile-delete request holds the deletion gate for its whole lifetime so
  // no concurrent route respawns a backend into the directory being removed;
  // every other request skips the gate entirely (release is a no-op then).
  const deletingProfile = profileNameFromDeleteRequest(request)
  const releaseProfileDeletion = deletingProfile ? profileDeletionGate.acquire(deletingProfile) : null
