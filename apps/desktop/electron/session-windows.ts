...
// `profile` preserves the legacy/profile-only backend fallback. `ownerRoute`
// carries exact source identity when the clicked row names one; it outranks the
// fallback during renderer boot and keeps same-id sessions on distinct sources
// in distinct native windows.
