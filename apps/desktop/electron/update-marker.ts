 ...
 * cannot keep resetting the 20-minute overdue threshold. When the updater
 * finishes it deletes the marker as before.
 ...
 *
 * Not used by the Windows script hand-off any more: there the desktop waits
 * for the script's own atomic claim instead of pre-writing the transient
 * cmd.exe wrapper's pid (see main.ts). The posix hand-off still bridges the
 * gap this way.
 */
export function writeUpdateMarker(
  ...
  // Only a proven-live holder has an age to inherit. A blocked (unreadable,
  // malformed, future-dated, cleanup-racing) marker carries no usable
  // timestamp, so the hand-off starts its own clock.
