...
def _detect_target_venv_holders(
    ...
    *,
    ...
    ``venv_dir`` overrides the in-tree ``venv``/``.venv`` lookup. The CLI
    updater passes ``hermes_constants.project_venv_dir(root)``, which also
    resolves an install whose interpreter lives outside the checkout
    (``$HERMES_HOME/venvs/<name>``, #116148); without it every holder running
    from that venv is missed. The standalone scanner and the Desktop carrier
    keep the in-tree lookup (``_validated_root`` requires an in-tree venv).
