...
export const AssistantMessage: FC<AssistantMessageProps> = props => {
  ...
  //
  // Every status-derived read has now moved off this level: the per-message
  // ones into AssistantStatusSlot / SettledChangedFiles / StreamingMarker, and
  // the last one — the inter-agent collapse gate — into
  // InterAgentAssistantMessage below. (useEnterAnimation used to hold `isRunning`
  // here too; it only consults `enabled` at mount, so AssistantMessageBody now
  // captures that once off the runtime with no subscription at all.) A pending
  // flip therefore descends into neither the parts subtree nor the
  // changed-files card from here.
  //
    ...
    >
      ...
      >
        ...
        {/* The activity row is mounted by the TAIL of the thread and decides
            ...
            arc border and Stop button said work was still happening. The
            tail/placeholder reads themselves live in AssistantStatusSlot so a
            pending flip stays below this root. */}
        <AssistantStatusSlot />
 ...
 *
 * Behaviour is identical to the inline expression it replaced, including the
 ...
 *
 * Note there is deliberately NO `running` gate on TurnActivityIndicator: the
 * row subscribes to the turn's own busy signal and decides for itself, so a
 * bubble sealed mid-flight (message.interim) at the tail keeps counting the
 * seconds the composer is still lit for.
 */
const AssistantStatusSlot: FC = () => {
  ...
  return isPlaceholder ? <ResponseLoadingIndicator /> : <TurnActivityIndicator />
}
