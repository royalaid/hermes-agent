...
describe('resumeSession failure recovery', () => {
  ...
  it('rejects a warm Todo projection after transcript authority changes', async () => {
    const runtimeIdByStoredSessionIdRef: MutableRefObject<Map<string, string>> = {
      current: new Map([['stored-1', 'runtime-warm-authority']])
    }
    const warmState = createClientSessionState('stored-1')
    ...
    const sessionStateByRuntimeIdRef: MutableRefObject<Map<string, ClientSessionState>> = {
      current: new Map([['runtime-warm-authority', warmState]])
    }
    const todoProjection = deferred<Awaited<ReturnType<typeof getSessionMessages>>>()
    ...
    vi.mocked(getSessionMessages).mockReturnValueOnce(todoProjection.promise)
    ...
    const requestGateway = vi.fn(async (method: string) => {
      if (method === 'session.activate') {
        return {
          ...
          resumed: 'stored-1',
          running: false,
          session_id: 'runtime-warm-authority',
          session_key: 'stored-1'
        } as never
      }
    ...
    const before = sessionStateByRuntimeIdRef.current.get('runtime-warm-authority')!
    const newerAuthority = {
      ...before,
      messages: [
        {
          id: 'new-authority',
          parts: [{ text: 'newer live state', type: 'text' as const }],
          role: 'assistant' as const
        }
      ],
    ...
    sessionStateByRuntimeIdRef.current.set('runtime-warm-authority', newerAuthority)
    todoProjection.resolve({
      messages: resumeCarrierMessages,
      pagination: { exhausted: true, has_more: false, limit: 32, next_before_id: null, returned: 1 },
      session_id: 'stored-1'
    } as never)
    await pending

    expect(sessionStateByRuntimeIdRef.current.get('runtime-warm-authority')).toBe(newerAuthority)
    expect($todosBySession.get()['runtime-warm-authority']).toBeUndefined()
  ...
  it('preserves a live warm-runtime stream that starts during Todo projection', async () => {
    const runtimeId = 'runtime-warm-live-projection'
    const runtimeIdByStoredSessionIdRef: MutableRefObject<Map<string, string>> = {
      current: new Map([['stored-1', runtimeId]])
    }
    const warmState = createClientSessionState('stored-1')
    ...
    const sessionStateByRuntimeIdRef: MutableRefObject<Map<string, ClientSessionState>> = {
      current: new Map([[runtimeId, warmState]])
    }
    const todoProjection = deferred<Awaited<ReturnType<typeof getSessionMessages>>>()
    ...
    const before = sessionStateByRuntimeIdRef.current.get(runtimeId)!
    const liveState: ClientSessionState = {
      ...
      messages: [
        ...before.messages,
        {
          ...
          parts: [{ text: 'new live answer', type: 'text' }],
          pending: true,
          role: 'assistant'
        }
      ],
    ...
    sessionStateByRuntimeIdRef.current.set(runtimeId, liveState)
    todoProjection.resolve({
      messages: [],
      pagination: { exhausted: true, has_more: false, limit: 32, next_before_id: null, returned: 0 },
      session_id: 'stored-1'
    ...
    expect(sessionStateByRuntimeIdRef.current.get(runtimeId)).toMatchObject({
      busy: true,
      streamId: 'assistant-stream-live',
      turnLive: true
    })
    expect(JSON.stringify(sessionStateByRuntimeIdRef.current.get(runtimeId)?.messages)).toContain('new live answer')
  })
