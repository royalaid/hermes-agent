...
function ChatRuntimeBoundary({
  ...
  if (transcriptMessagesRef.current.key !== transcriptIdentityKey || suppressMessages) {
    transcriptMessagesRef.current = { key: transcriptIdentityKey, messages: deduplicatedIncomingMessages, source: incomingMessages }
  } else if (transcriptMessagesRef.current.source !== incomingMessages) {
    const previousWindowFirst = windowStateRef.current.get(transcriptIdentityKey)?.state.window.messages[0]
    ...
    transcriptMessagesRef.current = { key: transcriptIdentityKey, messages: reconciled, source: incomingMessages }
  }
  ...
  const expandWindow = useCallback(() => {
    ...
    if (
      !windowStateRef.current.get(transcriptIdentityKeyRef.current ?? '')?.state.window.windowed &&
      runtimeId &&
      storedId &&
