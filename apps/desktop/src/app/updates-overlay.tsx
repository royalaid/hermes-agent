...
export function UpdatesOverlay() {
  ...
  const updateAvailable = status?.updateAvailable || behind > 0 || Boolean(status?.bundleOutOfSync)
...
function IdleView({
  ...
  const { title, body } = resolveUpdateCopy({
    target,
    shownItems,
    bundleRebuildOnly: target === 'client' && behind === 0 && Boolean(status.bundleOutOfSync),
    copy: u
  })
