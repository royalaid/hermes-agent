...
describe('reconcileActiveTranscript', () => {
  ...
  it('discards an active refresh when a queued user arrives during Todo projection', async () => {
    const fixture = makeRefresh()
    const todoRead = deferred<Awaited<ReturnType<typeof getSessionMessages>>>()

    vi.mocked(getLatestSessionMessages).mockResolvedValue({
      messages: Array.from({ length: 120 }, (_, index) => ({
        content: `ordinary persisted tail ${index}`,
        role: 'assistant' as const,
        timestamp: index + 1
      })),
      session_id: ACTIVE_STORED_ID
    } as never)
    vi.mocked(getSessionMessages).mockReturnValueOnce(todoRead.promise)
    ...
    publishSessionState(ACTIVE_RUNTIME_ID, fixture.state)
    todoRead.resolve({
      messages: carrierTranscript().messages,
      pagination: { exhausted: true, has_more: false, limit: 32, next_before_id: null, returned: 1 },
      session_id: ACTIVE_STORED_ID
    } as never)
    await request
    ...
    expect(fixture.updateSessionState).not.toHaveBeenCalled()
    expect($sessionStates.get()[ACTIVE_RUNTIME_ID]?.messages.map(message => message.id)).toEqual([
      `user-queued-${ACTIVE_RUNTIME_ID}`
    ])
    expect($todosBySession.get()[ACTIVE_RUNTIME_ID]).toBeUndefined()
    expect(fixture.signatureRef.current.size).toBe(0)
