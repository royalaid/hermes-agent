...
pub struct StartBootstrapArgs {
    /// Optional override for the GitHub `owner/repository` that provides the
    /// install script. Defaults to the build-time `BUILD_PIN_REPOSITORY`.
...
fn build_pin_args(script: &install_script::ResolvedScript) -> Vec<String> {
    let mut out = Vec::new();
    if let Some(r) = &script.repository {
        if r != "NousResearch/hermes-agent" {
...
mod tests {
    ...
    #[test]
    fn build_pin_args_forwards_repository_before_ref_pins() {
