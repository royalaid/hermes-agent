...
fn write_bootstrap_complete_marker(install_root: &Path, pin: &Pin) -> Result<serde_json::Value> {
    ...
    let marker = serde_json::json!({
        ...
        "pinnedRepository": pin.repository.clone(),
...
fn build_pin_args(script: &install_script::ResolvedScript) -> Vec<String> {
    let mut out = Vec::new();
    if let Some(repository) = &script.repository {
        let repository_flag =
            if script.path.extension().and_then(|value| value.to_str()) == Some("ps1") {
                "-Repository"
            } else {
                "--repository"
            };
        out.push(repository_flag.to_string());
...
mod tests {
    ...
    #[test]
    fn build_pin_args_includes_repository_with_commit_and_branch() {
        let script = install_script::ResolvedScript {
            path: PathBuf::from("install.ps1"),
            source: install_script::ScriptSource::Downloaded,
            repository: Some("royalaid/hermes-agent".to_string()),
            commit: Some("51962fc0fb0dc468b5c8ab66c43d9a6a0e3dce3a".to_string()),
    ...
    #[test]
    fn build_pin_args_uses_posix_repository_flag_for_install_sh() {
        let script = install_script::ResolvedScript {
            path: PathBuf::from("install.sh"),
            source: install_script::ScriptSource::Downloaded,
            repository: Some("royalaid/hermes-agent".to_string()),
            commit: None,
            branch: Some("local/openai-native-windows".to_string()),
    ...
    fn bootstrap_complete_marker_uses_desktop_compatible_schema() {
        ...
        assert_eq!(from_disk["pinnedRepository"], "royalaid/hermes-agent");
        assert!(
