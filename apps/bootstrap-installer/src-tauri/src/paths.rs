...
/// mid-update re-locks the venv shim outside the updater's exact-child Job.
/// The updater never kills an unproven holder, so it fails closed instead of
/// mutating an installation whose files remain locked (#50238).
///
