...
use crate::powershell::{read_decoded_line, DRAIN_GRACE};
...
/// update" case — malformed bytes, a dead pid, or a pid proven reused —
/// matching `readLiveUpdateMarker` in the Electron gate. An unreadable pid is
/// still an owner: inspection failure is not proof of death. Age never enters
/// the decision; a live claim past the ceiling stays authoritative. Never
/// panics.
///
/// Callers pass the exact bytes they are about to compare-and-swap against, so
/// ownership is decided from the same snapshot that gets published over rather
/// than from a second, racy read.
///
/// Self-PID is reported like any other live owner so `acquire` can *adopt* the
/// desktop's pre-written claim without refreshing its acquisition time
/// (#74761). Without that adoption, `acquire` would see a live owner that is
/// itself, abort with "Another Hermes update is already running", and the
/// desktop would relaunch and retry forever. A foreign live pid (e.g. a
/// dashboard-spawned `hermes update`) still blocks.
...
impl UpdateMarkerGuard {
    ...
    ///
    /// Publishing the claim is fail-closed: it used to be best-effort ("no
    /// marker => proceed"), but Electron now uses this exact PID claim as its
    /// positive updater acknowledgement, so a silent write failure would let
    /// mutation begin while the Desktop still owns install-file handles.
    fn acquire(path: PathBuf) -> std::result::Result<Self, UpdateMarkerAcquireError> {
        ...
        if let Some(owner) = prior.as_deref().and_then(live_marker_owner_from_raw) {
            if owner.pid == pid {
                ...
                // re-entrant, and the desktop may have raced ahead and
                // pre-written our pid (#74761). Adopt that claim verbatim:
                // rewriting started_at here lets retries reset a wedged
                // updater's age, and republishing would CAS over bytes that
                // are already ours.
...
async fn run_streamed_with_timeout(
    ...
    // Streamed by hand rather than through `pump_child`: this stage has to
    // interleave the bridge-quiesce lease poll (and its bounded stage
    // deadline) with the same select that reads the pipes, which the shared
    // helper cannot express. The contract that matters is unchanged — the exit
    // status comes from waiting on the *process*, never from pipe EOF.
    ...
    // #90455: a descendant that outlived the child can still hold the pipes
    // open, so this drain is bounded by the same grace the shared `pump_child`
    // helper uses. Two things then have to happen: the dropped tail of the log
    // is announced out loud (a silently truncated log is indistinguishable
    // from an empty one), and the stage still fails closed — a descendant that
    // outlived the mutating child may not be finished with the install tree.
    let drain = async {
    ...
    if tokio::time::timeout(DRAIN_GRACE, drain).await.is_err() {
        ...
        return Err(anyhow!(
            "the updater child pipes did not close after process exit"
        ));
    }
