...
def _extensionless_media_matches(masked: str, *, include_unavailable: bool = False):
    ...
    for match in MEDIA_EXTENSIONLESS_TAG_RE.finditer(masked):
        ...
        if path and _path_lacks_deliverable_extension(path):
            resolved = _match_extensionless_path(
                masked, match, include_unavailable=include_unavailable)
            if resolved is not None:
                yield match, resolved[0], resolved[1]


def _real_media_tag_spans(masked: str, *, include_unavailable: bool = False) -> list:
    ...
    spans.extend(
        ...
        for match, _, end in _extensionless_media_matches(
            masked, include_unavailable=include_unavailable)
    )
    return spans
...
class BasePlatformAdapter(ABC):
    ...
    async def _extract_response_content(
        ...
        pre_extract = response
        # Resolve attachment policy in the active source profile while retaining unavailable
        # claimed-result intents for later send-time validation.
        with self._media_delivery_scope(event.source):
            if include_unavailable_attachments:
                media_files, response = BasePlatformAdapter.extract_media(
                    response, include_unavailable=True)
            else:
            ...
            # Strip remaining internal directives; unknown-extension MEDIA tags stay available
            # to the bare-path detector below.
            ...
            if not response:
                ...
                extracted = await self._extract_response_content(
                    ...
                    claimed_parts_snapshot=(
                        getattr(event, "_hermes_claimed_response_parts_snapshot", None)
                        if _delivery_owned_obligation_id else None))
