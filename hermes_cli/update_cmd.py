...
from hermes_cli.update_success_receipt import (
    _IDENTIFIER_RE,
    _SHA_RE,
...
def _classify_fetch_failure(stderr: str, remote: str = "origin") -> str:
    ...
    ``remote`` names the remote the fetch was actually scoped to, so the
    generic fallback doesn't tell a fork user to blame "origin" when the
    update fetched from a different configured remote. Defaults to "origin"
    for callers that have no resolved target to report.
    """
...
def _print_fetch_failure(stderr: str, remote: str = "origin") -> None:
    ...
    print(_classify_fetch_failure(stderr, remote))
    if stderr:
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    ...
    if verify_result.returncode != 0:
        print(f"✗ Branch '{branch}' not found on {target.remote}.")
        sys.exit(1)
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        if fetch_result.returncode != 0:
            _print_fetch_failure(fetch_result.stderr, update_target.remote)
            sys.exit(1)
        ...
        if current_branch != branch:
            ...
            if checkout_result.returncode != 0:
                ...
                # it up from the exact tracking ref just refreshed. This is
