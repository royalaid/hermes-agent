...
# The fork carves the updater into update_deferred_gateway/update_quiesce/
# update_readiness/update_transaction modules; those do not exist here, so
# every symbol it imports from them stays defined in this module.  Only the
...
def _resolve_update_target(
    ...
    """Resolve one validated branch and its configured fetch remote."""
    command_env = _sanitized_git_env() if env is None else env
    ...
    if configured_remote.returncode == 0:
        remote = configured_remote.stdout.strip() or "origin"
    elif configured_remote.returncode == 1:
        remote = "origin"
    else:
        raise ValueError(f"could not resolve update remote for branch: {branch}")
    if not _is_safe_remote_name(remote):
        raise ValueError("invalid update remote")
...
# ``_sanitize_update_receipt`` now lives in ``hermes_cli.update_receipt`` (see
# the module import above) and is re-exported here: same contract, but the
# receipt SHAs are matched at exact widths instead of the loose short-SHA form.
...
def _assess_parked_branch_switch(
    ...
    target_ref = tracking_ref or f"origin/{target_branch}"
    cherry = subprocess.run(
        git_cmd + ["cherry", target_ref],
        cwd=cwd, capture_output=True,
        text=True, encoding="utf-8", errors="replace",
    )
...
def _print_parked_branch_skip_warning(
    ...
    if reason == "dirty":
        ...
        why = (
            f"the branch state could not be verified against {target_ref}"
        )
    ...
    if behind is not None and behind > 0:
        print(
            ...
            f"{target_ref} — the code you are running is stale."
        )
    print()
...
def _print_parked_branch_kept_notice(
    ...
    print(
        ...
        f"{unmerged_count} commit(s) not merged into {target_ref}."
    )
    print(
...
def _update_via_zip(
    ...
    *,
    ...
    # A failed Node refresh OR a failed Desktop rebuild is non-fatal for the
    # Python side but must not print "✓ Update complete!" (#88251) — and must
    # not write a success receipt either. Only the fully-clean path records
    # the archive-mode receipt and prints completion.
    if node_failures or not desktop_build_ok:
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    ...
    "main"; callers can pass another branch to ask whether its configured
    fetch remote has new commits without performing the update.
    ...
    # so scope the fetch to the validated source/destination refspec.
    ...
    print(f"→ Fetching from {update_target.remote}...")
    fetch_result = subprocess.run(
        git_cmd
        + ["fetch"]
        + depth_args
        + ["--", update_target.remote, update_target.refspec],
        cwd=_m().PROJECT_ROOT,
        capture_output=True,
        text=True,
        encoding="utf-8",
    ...
    compare_branch = update_target.tracking_ref
...
def _format_venv_python_holders_message(matches: list[tuple[int, str, str]]) -> str:
    ...
    for pid, name, cmdline in matches[:6]:
        ...
        if command == "serve":
            hint = "  ← Hermes backend (if the Desktop app is open, close it)"
        elif command == "dashboard":
            hint = (
                "  ← hermes dashboard "
                "(stop it: hermes dashboard stop, or close that terminal)"
            )
...
def _capture_reapable_backend_identity(pid: object) -> _VerifiedProcessIdentity | None:
    """Prove a rung-supplied numeric PID is still a live Hermes backend.

    The ledger and hand-off rungs (``_ledger_reapable_backend_pids`` /
    ``_handoff_reapable_backend_pids``) hand down bare PIDs because their
    authority is NOT parent-death archaeology: the ledger proves ownership
    from the process's own registration, and the hand-off rung reaps
    backends that still have a live parent on purpose — that is the whole
    swarm case it exists for. Neither can produce an ``orphan_backend``
    identity, and ``_revalidate_orphan_backend_identity`` would (correctly,
    for its own kind) refuse them.

    A discovery-time integer is still not kill authority. Re-read the PID
    here, immediately before the stop, and require that it is right now a
    live Hermes ``serve``/``dashboard`` backend with a readable creation
    time — the same exact-token matcher the orphan path uses, never a
    cmdline substring. That closes the PID-reuse window between the scan
    and the ``taskkill`` without discarding the rung's own proof.

    Returns ``None`` (refuse) for anything unreadable, non-numeric, or no
    longer a backend. Never raises.
    """
    try:
        import psutil  # type: ignore
        from hermes_cli._scan_venv_blockers import _hermes_cli_command

        numeric_pid = int(pid)  # type: ignore[arg-type]
        process = psutil.Process(numeric_pid)
        created_at = float(process.create_time())
        argv = [str(value) for value in process.cmdline()]
        if (
...
def _revalidate_reapable_backend_identity(
    identity: _VerifiedProcessIdentity,
) -> bool:
    """Re-prove one authorized identity immediately before its tree stop.

    Dispatches on ``kind`` so each rung is re-checked against its OWN claim:
    an ``orphan_backend`` must still be parentless (or PID-reuse-proof),
    while a ``verified_backend`` promoted from the ledger/hand-off rungs
    must still be the same live backend process it was when captured.
    Unknown kinds fail closed.
    """
    if identity.kind == "orphan_backend":
        return _revalidate_orphan_backend_identity(identity)
    if identity.kind == "verified_backend":
        current = _capture_reapable_backend_identity(identity.pid)
        return (
            current is not None
            and abs(current.created_at - identity.created_at) <= 0.001
        )
    return False


def _stop_process_trees(
    identities: list[_VerifiedProcessIdentity | int],
) -> None:
    ...
    Accepts either a ``_VerifiedProcessIdentity`` (the orphan rung, which
    proves parent-death at classification time) or a bare PID from the
    ledger/hand-off rungs, whose authority comes from the spawn ledger and
    the hand-off contract instead. A bare PID is never killed on the scan's
    say-so: it must first re-prove itself a live Hermes backend via
    ``_capture_reapable_backend_identity``. Either way the identity is
    re-checked against its own claim immediately before the ``taskkill``.
    """
    for candidate in identities:
        identity = candidate
        if not isinstance(identity, _VerifiedProcessIdentity):
            identity = _capture_reapable_backend_identity(candidate)
            if identity is None:
                logger.warning(
                    "Refusing bare-PID process-tree stop without identity proof"
                )
                continue
        if not _revalidate_reapable_backend_identity(identity):
            logger.warning(
...
def _cmd_update_impl(
    ...
    *,
    ...
    if not deferred_gateway_resume:
        ...
        # Publish the plan instead of registering an atexit resume: the outer
        # command resumes it in its own ``finally`` once the mutation Job has
    ...
    selected_remote = update_target.remote if update_target is not None else "origin"
    selected_remote_url = _get_remote_url(
        git_cmd, _m().PROJECT_ROOT, selected_remote
    )
    is_fork = _is_fork(selected_remote_url)
    ...
    try:
            ...
            if switch_block_reason.startswith("unmerged:"):
                ...
                if _in_place_configured and not switch_branch:
                    ...
                    if verify_ref.returncode != 0:
                        print(
                            f"✗ Branch '{branch}' does not exist locally or on "
                            f"{update_target.remote}."
                        )
                        sys.exit(1)
                    ...
                    print(
                        ...
                        "(no branch switch; local commits preserved)."
                    )
                else:
        ...
        if commit_count > 0 and apply_is_shallow:
            ...
            target_sha = subprocess.run(
                git_cmd + ["rev-parse", update_target.tracking_ref],
                cwd=_m().PROJECT_ROOT, capture_output=True,
                text=True, encoding="utf-8", errors="replace",
            ).stdout.strip()
            counted = _github_compare_behind(head_sha, target_sha)
        ...
        before_fork_sync: str | None = None
        if (
            commit_count == 0
            and is_fork
            and branch == "main"
        ...
        if commit_count == 0:
            ...
            # already ran above, before the commit count was consumed: that
            # block performs the same _sync_with_upstream_if_needed and sets
            # fork_sync_performed.  Repeating it here would sync twice, and
            # unlike that block it would ignore the origin-owned-main check.
            ...
            # exist on the original branch — applying while still on the
            # update target can create conflicts the original branch does not
            # have.
            ...
            if handed_off_sync:
                ...
                # ``_venv_core_imports_healthy`` is tri-state: ``None`` means the
                # probe could not establish health, which is not the same claim
                # as proven breakage. Repair either way, but say which one it is.
            ...
            if pull_result.returncode != 0:
                ...
                if _cur_branch and _cur_branch != branch:
                    print(
                        ...
                        f"merging {update_target.tracking_ref} instead of "
                    ...
                    merge_result = subprocess.run(
                        git_cmd + ["merge", "--no-edit", update_target.tracking_ref],
                        cwd=_m().PROJECT_ROOT,
                        capture_output=True,
                        text=True, encoding="utf-8", errors="replace",
                        env=git_env,
                    ...
                    if merge_result.returncode != 0:
                        ...
                        print(
                            ...
                            f"git merge {update_target.tracking_ref}"
                        )
                        print(
        ...
        # claiming success. When fork synchronization already moved HEAD,
        # ``pre_pull_sha`` is the pre-sync baseline so this comparison, the
        # dependency diff, and syntax rollback all cover the first mutation.
        ...
        if pre_pull_sha and post_pull_sha == pre_pull_sha:
            ...
            print(
                ...
                f"{update_target.tracking_ref} advanced but the working tree "
                "stayed put."
            )
            print(
        ...
        if (
            ...
            print()
            print(
                f"✗ Update pulled {update_target.tracking_ref}, but the checkout is on "
        ...
        # Fork upstream sync logic (only for main branch on forks).  Keep the
        # legacy upstream reconciliation origin-specific: a main branch
        # explicitly owned by another remote remains isolated, and a sync the
        # up-to-date path already performed is not repeated.
        if (
            is_fork
            and branch == "main"
            and update_target.remote == "origin"
        ...
        # A failed Node refresh OR a failed Desktop rebuild is non-fatal for
        # the Python side but must not print "✓ Update complete!" (#88251).
        # Everything past that gate still has to prove runtime health and the
        # installed Git identity before a success receipt is written.
        if node_failures or not desktop_build_ok:
            _print_update_summary(
                node_failures=node_failures,
                desktop_build_ok=desktop_build_ok,
        ...
        try:
            ...
            # all_profiles=True keeps sibling-profile launchd gateways out of
            # the manual set so the sweep cannot misclassify them (#73626);
            # _legacy_manual_gateway_pids then yields nothing on the Windows /
            # deferred paths, where the outer command already owns the fleet.
            ...
            # Nothing was killed here means nothing can be stuck: skip the
            # 3s wait and the fleet-wide rescan entirely.
            if killed_pids:
                try:
                    _time.sleep(3.0)
