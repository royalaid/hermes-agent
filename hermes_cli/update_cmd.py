...
def _resolve_update_target(
    ...
    """Resolve one branch remote/ref contract for fetch and every consumer."""
    command_env = _sanitized_git_env() if env is None else env
    ...
    # A dot remote tracks another local branch and has no remote namespace to
    # refresh. Managed updates need an actual fetch target, so use origin.
    if not remote or remote == ".":
        remote = "origin"
    if not _is_safe_remote_name(remote):
        raise ValueError(f"invalid update remote: {remote}")
