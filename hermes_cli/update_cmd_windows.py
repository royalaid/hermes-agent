...
def _detect_venv_python_processes(
    *,
    ...
    # The live venv, not ``<root>/venv``: ``project_venv_dir`` also resolves an out-of-tree
    # interpreter (``$HERMES_HOME/venvs/<name>``) for the checkout this code runs from (#116148).
    from hermes_constants import project_venv_dir
    live_venv = project_venv_dir(root)
    if live_venv is not None:
        kwargs["venv_dir"] = live_venv
    try:
